...
for ((i=0; i<${#samples_and_controls[@]}; i+=2)); do
  ...
  control="${samples_and_controls[i+1]}"
    ...
    bigwigCompare -b1 output/bigwig_histone/${sample}.dupmark.sorted.bw \
        -b2 output/bigwig_histone/${control}.dupmark.sorted.bw \
        --operation subtract \
        -o output/bigwig_histone_IggNorm_subtract/${sample}_subtract.bw
done
