...
for ((i=0; i<${#samples_and_controls[@]}; i+=2)); do
  ...
  control="${samples_and_controls[i+1]}"
    ...
    bigwigCompare -b1 output/bigwig/${sample}.unique.dupmark.sorted.bw \
        -b2 output/bigwig/${control}.unique.dupmark.sorted.bw \
        --operation subtract \
        -o output/bigwig/${sample}_subtract.unique.dupmark.sorted.bw
done
