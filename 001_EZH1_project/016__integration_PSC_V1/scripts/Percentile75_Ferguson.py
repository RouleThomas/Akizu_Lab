...
output_file = "output/bigwig/Ferguson_SF_75.txt"


# Initialize a dictionary to store 75th percentile values
percentile_dict = {}
...
for sample_name in input_list:
    ...
    # Calculate the 75th percentile of the 'score' column
    percentile_75 = np.percentile(data["score"], 75)
    percentile_dict[sample_name] = percentile_75
    print(f"{sample_name}: 75th percentile = {percentile_75}")

# Save the 75th percentile values to a file
with open(output_file, "w") as f:
    for sample, percentile in percentile_dict.items():
        f.write(f"{sample}\t{percentile}\n")

print(f"75th percentile values saved to {output_file}")
