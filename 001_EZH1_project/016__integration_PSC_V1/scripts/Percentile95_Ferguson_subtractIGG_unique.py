...
output_file = "output/bigwig/Ferguson_SF_95_subtract_unique.txt"


# Initialize a dictionary to store 95th percentile values
percentile_dict = {}

# Loop through each sample
for sample_name in input_list:
    input_file = os.path.join(local_maxima_dir, f"{sample_name}_subtract_unique_local_maxima.bed")
    ...
    # Calculate the 95th percentile of the 'score' column
    percentile_95 = np.percentile(data["score"], 95)
    percentile_dict[sample_name] = percentile_95
    print(f"{sample_name}: 95th percentile = {percentile_95}")

# Save the 95th percentile values to a file
with open(output_file, "w") as f:
    for sample, percentile in percentile_dict.items():
        f.write(f"{sample}\t{percentile}\n")

print(f"95th percentile values saved to {output_file}")
