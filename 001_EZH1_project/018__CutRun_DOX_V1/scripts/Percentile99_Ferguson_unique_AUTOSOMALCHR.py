...
output_file = "output/bigwig/Ferguson_SF_99_unique_AUTOSOMALCHR.txt"
...
for sample_name in input_list:
    ...
    # keep autosomes only ###################
    autosomes = [f"chr{i}" for i in range(1,23)]  # human; mouse: range(1,20)
    data = data[data["chrom"].isin(autosomes)]
    ######################################
