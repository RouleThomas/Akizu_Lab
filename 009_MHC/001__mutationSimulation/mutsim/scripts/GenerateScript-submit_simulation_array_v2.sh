...
SIGFILE="/scr1/users/roulet/Akizu_Lab/009_MHC/001__mutationSimulation/mutsim/signatures/COSMIC_v3.4_SBS_GRCh38.txt"

# Where to write all generated scripts
OUTDIR="scripts/submit_all_signatures"
mkdir -p "$OUTDIR"
...
for SIG in $SIGS; do
  SCRIPT="$OUTDIR/submit_simulation_array_v2-${SIG}.sh"
  cat > "$SCRIPT" <<EOF
#!/bin/bash
