...
# Extract stabilization points per experimental signature (non-SBS)
...
for ax, (metric, title) in zip(axs, METRICS.items()):
    ...
    ax.set_ylabel("Signature Count", fontsize=9)
...
plt.tight_layout()
plt.savefig(INPUT_DIR / "stabilization_summary_experimental_all.pdf")
plt.close()
