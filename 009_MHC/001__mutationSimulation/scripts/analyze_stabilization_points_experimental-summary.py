...
# Extract stabilization points per non-SBS signature and metric
for sig_folder in sorted(INPUT_DIR.iterdir()):
    if not sig_folder.is_dir() or sig_folder.name.startswith("SBS"):
        continue
...
for metric, stab_dict in stabilization_points.items():
    ...
    plt.ylabel("Number of Experimental Signatures")
    plt.xticks(rotation=45)
    plt.tight_layout()

    plt.savefig(INPUT_DIR / f"stabilization_summary_experimental_{metric}.pdf")
    plt.close()
