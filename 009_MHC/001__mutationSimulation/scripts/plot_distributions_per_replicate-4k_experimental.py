...
def consequence_to_binary(df):
    ...
    df["frac_stop"] = (df["consequence"] == "stop").astype(int)
    return df

# Iterate over all folders EXCEPT those starting with SBS
for signature_folder in sorted(BASE_DIR.iterdir()):
    if not signature_folder.is_dir() or signature_folder.name.startswith("SBS"):
        continue

    n4000_folder = signature_folder / "n_4000"
    if not n4000_folder.exists():
        continue

    pdf_path = n4000_folder / f"{signature_folder.name}_replicate_score_distributions.pdf"
    with PdfPages(pdf_path) as pdf:

        # Find all replicates
        for parquet_file in sorted(n4000_folder.glob("rep_*.annot.parquet")):
            ...
            for i, (col, title) in enumerate(SCORE_COLUMNS.items()):
                ...
                ax.scatter([col] * len(df), df[col], color="black", alpha=0.3, s=5)
                ...
                if col in {"frac_syn", "frac_missense", "frac_stop"}:
            ...
            fig.suptitle(f"{signature_folder.name} - {replicate_name}: Mutation Score Distributions", fontsize=12)
            plt.tight_layout(rect=[0, 0.03, 1, 0.95])
            pdf.savefig(fig)
            plt.close()

    print(f"✅ Finished plotting {signature_folder.name}")
