...
RE_STOP_RETAINED = re.compile(r'\b(?:stop[_ ]?retained)\b', re.I)


def strict_stop_lost_mask(df):
    """
    Return boolean mask for TRUE stop-lost (stoploss):
      - prefer explicit effect labels (stop_lost/stoploss),
      - else derive from AA fields if present (aa_ref == '*' & aa_alt != '*'),
      - else try HGVSp-like strings (rarely reliable for stoploss without parsing),
      - else fall back to exact consequence labels if present.
    """
    ...
    # 3) last resort: consequence column
    if 'consequence' in df.columns:
        s = df['consequence'].astype(str).str.lower()
