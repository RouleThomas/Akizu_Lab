...
for parquet in sorted(PARQ_DIR.glob("chr*.parquet")):
    ...
    refs = INT2BASE[tbl["ref_base"].to_numpy()]  # literal coding-strand base
    strands = tbl["strand"].to_numpy()

    for chrom, pos, ref, strand in zip(chrs, poss, refs, strands):

        try:
            tri = fasta.fetch(chrom, pos - 1, pos + 2).upper()  # get +1/-1 on both sides
        except Exception:
            row_counter += 1
            continue
        ...
        for alt in ("AGT" if ref_base == "C" else "ACG"):
            ...
            ctx_id = CTX2ID[ctx]
            context_rows[ctx_id].append(row_counter)
...
full_index: dict[int, np.ndarray] = {
    ctx_id: np.array(context_rows.get(ctx_id, []), dtype=np.int32)
    for ctx_id in range(96)
}
