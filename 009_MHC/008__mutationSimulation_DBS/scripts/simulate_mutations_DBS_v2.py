...
class ExomeSampler:
    ...
    def __init__(
        ...
        # IMPORTANT: must match the index-builder order exactly
        shards = sorted(parquet_dir.glob("chr*.parquet"))
        if not shards:
            raise FileNotFoundError(f"No chr*.parquet shards found in {parquet_dir}")
