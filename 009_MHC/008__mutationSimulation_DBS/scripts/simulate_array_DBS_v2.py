...
from simulate_mutations_DBS_v2 import ExomeSampler, load_signatures, get_signature_probs
...
def main():
    ...
    # context names order (used to map indices)
    ...
    sampled_df["context_id"]   = np.asarray(ctx_ids, dtype=int)
    sampled_df["context_name"] = [context_names[i] for i in sampled_df["context_id"]]

    sampled_df["signature"]   = sig
    sampled_df["rep"]         = int(args.rep)
    sampled_df["seed"]        = int(seed)
    sampled_df["n_requested"] = int(args.n)
    sampled_df["n_returned"]  = int(len(sampled_df))

    # HARD FAIL if not exact (catches pool-size / replace=False bugs immediately)
    if len(sampled_df) != args.n:
        raise RuntimeError(
            f"DBS sampling returned {len(sampled_df)} but requested {args.n}. "
            f"Likely some context pools are smaller than needed and sampling used replace=False."
        )

    pq.write_table(pa.Table.from_pandas(sampled_df), parquet_path, compression="zstd")

    ctx_counts = pd.Series(sampled_df["context_id"]).value_counts().sort_index()
