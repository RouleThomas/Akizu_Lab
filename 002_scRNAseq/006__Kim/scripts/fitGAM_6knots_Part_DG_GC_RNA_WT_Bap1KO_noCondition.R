...
traj_Part_DG_GC_noCondition <- fitGAM(
     counts = sub_counts, 
     pseudotime = sub_pseudotimes,
     cellWeights = sub_weights,
     conditions = NULL,  # HERE we remove condition
     nknots = 6,
     sce = TRUE
   )

saveRDS(traj_Part_DG_GC_noCondition, file = "output/condiments/traj_Part_DG_GC_noCondition.rds")
