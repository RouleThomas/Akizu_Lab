...
DefaultAssay(WT_Kcnc1_p14_CX_1step) <- "RNA"
...
shared_features <- intersect(
  rownames(Yao_Cortex),
  rownames(WT_Kcnc1_p14_CX_1step)
)
...
WT_Kcnc1_p14_CX_1step <- AddMetaData(WT_Kcnc1_p14_CX_1step, metadata = predictions)


# Step 3: Project query onto reference UMAP
WT_Kcnc1_p14_CX_1step <- MapQuery(
  anchorset = anchors,
  reference = Yao_Cortex,
  query = WT_Kcnc1_p14_CX_1step,
...
# Step 4: Export
pdf("output/seurat/UMAP_Yao_Cortex-reference_query_overlay-order2.pdf", width = 30, height = 7)
(p1 | p2 | g_overlay)
dev.off()
...
saveRDS(Yao_Cortex, file = "output/seurat/Yao_Cortex-10X_nuclei_v3_AIBS-30dim-order2.rds") 
saveRDS(WT_Kcnc1_p14_CX_1step, file = "output/seurat/WT_Kcnc1_p14_CX_1step-10X_nuclei_v3_AIBS-30dim-order2.rds") 
