...
WT_Kcnc1_CB <- slingshot(WT_Kcnc1_CB, reducedDim = 'UMAP',
                 ...
                 start.clus = c("MLI1_3"), end.clus = c("MLI1_1") ,approx_points = 100, extend = 'n', stretch = 1)
...
curves <- bind_rows(lapply(sdss, slingCurves, as.df = TRUE),
                    .id = "condition")
pdf("output/condiments/UMAP_trajectory_WT_Kcnc1_CB-dim40kparam15res03-START-MLI1_3-END-MLI1_1-points100extendnstretch1.pdf", width=6, height=5)
...
# Save RData
save.image(file = "output/condiments/pseudotime_allCells_v1_MLI1.RData")
